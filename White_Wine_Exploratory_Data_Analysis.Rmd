...
ggplot(wine, aes(y=volatile.acidity, x=alcohol, color=quality.cut,
                 size=quality)) + geom_jitter(alpha=0.5)
```
...
```{r echo=FALSE}
ggplot(wine, aes(y=pH, x=alcohol, color=quality.cut, size=quality)) +
...
# We are excluding top and bottom 1% observations as outliers here
# x and y intercept line values are decided based on our earlier observations
# during multivariate analysis
#
ggplot(data=wine, aes(x=alcohol, y=volatile.acidity, size=quality, 
                      color=quality.cut)) +
